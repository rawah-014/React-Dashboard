...
import { getAdminDeliverys, deleteDelivery, clearErrors } from '../actions/deliveryActions'
import { DELETE_DELIVERY_RESET } from '../constants/deliveryConstants'

const DeliveryList = ({ history }) => {

    const alert = useAlert();
    const dispatch = useDispatch();

    const { deliverys ,loading, error } = useSelector(state => state.deliverys);
    const { error: deleteError, isDeleted } = useSelector(state => state.delivery)

    useEffect(() => {
        dispatch(getAdminDeliverys());
